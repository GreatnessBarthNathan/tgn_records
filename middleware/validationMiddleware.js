...
// RC VALIDATION
export const validateRCInput = withValidationErrors([
  body('name').notEmpty().withMessage('name is required'),
  body('address').notEmpty().withMessage('address is required'),
]);

export const validateRCIdParam = withValidationErrors([
  param('id').custom(async (id, { req }) => {
    const isValidId = await mongoose.Types.ObjectId.isValid(id);
    if (!isValidId) throw new BadRequestError('invalid mongoose id');

    const rc = await RC.findOne({ _id: id });
    if (!rc) throw new NotFoundError('this record is not available');
  }),
]);
